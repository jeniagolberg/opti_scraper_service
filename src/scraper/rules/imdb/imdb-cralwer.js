...
const getDataByAncestors = async (element, ancestors) => {
    try{
        return await page.$eval(ancestors, el => el.innerText);
    } catch (err) {
...
const extractData = async (page) => {
    ...
    return {budget,cwg,releaseDate,runtime,fullName,rating,genres}
}
...
module.exports = { 
     ...
     */
    extractImdbData: async function(page) {
        console.log(await extractData(page));
    },
    /**
